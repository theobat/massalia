...
class Schema a where
  parseQuery :: Context -> JsonMassaliaTree -> Either ParseQueryError (Selection a)
  default parseQuery :: (Generic (Selection a), GSchema (Rep (Selection a))) =>
    Context -> JsonMassaliaTree -> Either ParseQueryError (Selection a)
  parseQuery ctx = fmap to . gParseQuery Proxy ctx
...
instance Show ParseQueryError where
  show PQError{pqe_ctx, pqe_msg} = Text.unpack $
    intercalate "." ("root" : reverse pqe_ctx) <> ": " <> pqe_msg
mkFieldErr :: Context -> Text -> Text -> ParseQueryError
...
class GSchema f where
  gParseQuery :: Proxy (f a) -> Context -> JsonMassaliaTree -> Either ParseQueryError (f a)

instance GSchema a => GSchema (M1 D s a) where
  gParseQuery _ ctx = fmap M1 . gParseQuery (Proxy @(a _)) ctx
instance GSchema a => GSchema (M1 C s a) where
  gParseQuery _ ctx = fmap M1 . gParseQuery (Proxy @(a _)) ctx
instance (Selector s) => GSchema (M1 S s (K1 i (ScalarSelect m a))) where
  gParseQuery _ ctx t = M1 . K1 <$> case lookupChildren fieldName t of
      Nothing            -> Right SDiscard
      Just x | isLeaf x  -> Right SPick
             | otherwise -> Left (mkFieldErr ctx fieldName "must be a leaf node")
...
instance (Selector s, Schema a) => GSchema (M1 S s (K1 i (ObjSelect m (Selection a)))) where
  gParseQuery _ ctx t = M1 . K1 <$> case lookupChildren fieldName t of
      ...
      Just x | isLeaf x  -> Left (mkFieldErr ctx fieldName "cannot be a leaf node, must specify children")
             | otherwise -> OPick <$> parseQuery (fieldName : ctx) x
    where fieldName = Text.pack $ selName (proxySelName :: (M1 S s _ _))
instance (GSchema a, GSchema b) => GSchema (a :*: b) where
  gParseQuery _ ctx t = liftA2 (:*:) (gParseQuery (Proxy @(a _)) ctx t) (gParseQuery (Proxy @(b _)) ctx t)
